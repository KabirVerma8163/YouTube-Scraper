...
def get_youtube_data(auth_json):
    ...
    with sync_playwright() as p:
        ...
        return_array.append(page.locator("div#contents.style-scope.ytd-section-list-renderer").inner_html())
        # A = page.locator("div#contents.style-scope.ytd-section-list-renderer")
        # A.text_content()
        # for section in sections_to_search:
        #     return_array.append(section.text_content())
        browser.close()

    return return_array

def save_youtube_data(auth_json, file_name):
    data = get_youtube_data(auth_json)
    with open(file_name, "w") as file:
        for val in data:
            file.write(val)

# pprint.pprint(get_youtube_data("auth.json")[0])